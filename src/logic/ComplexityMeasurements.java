...
//				System.out.println(ex.getMessage());
	...
	public static Map<String, Integer> calculateInheritanceMeasurements(ComplexityMeasurements cm) throws IOException, ClassNotFoundException {
		...
		Map<String, Integer> class_inheritance_count = new HashMap<>();
...
//			System.out.println("Control structure measurements: " + control_points);
	...
	public static List<ProgramStatementComplexity> calculateNestingLevelMeasurement(ComplexityMeasurements cm) throws IOException, NullPointerException {
...
//			System.out.println("Nested count for line no. " + line_count + ": " + nested_count);
			...
			ProgramStatementComplexity psc = new ProgramStatementComplexity();
			psc.setLineNumber(line_count);
			psc.setNested_level_count(nested_count);
	...
	public static List<String> calculateRecursionMeasurements(ComplexityMeasurements cm) throws IOException {
		...
		List<String> recursive_methods = new ArrayList<>();
		...
		while ((statement = br.readLine()) != null) {
			...
			if (!statement.contains("\"") && statement.contains(" ")) {
				...
				for (String return_type: ComplexitySettings.RETURN_TYPES) {
					if (parts[0].equals(ComplexitySettings.PUBLIC) || parts[0].equals(ComplexitySettings.STATIC) || parts[0].equals(return_type)) {
...
//						System.out.println("Method name: " + method[0].trim());
	...
	public static List<ProgramStatementComplexity> getInheritanceCountForStatement(ComplexityMeasurements cm, Map<String, Integer> class_inheritance_counts) throws IOException {
		BufferedReader br = cm.getBr();
		List<String> user_classes = cm.getUser_defined_classes();
		String statement = null;
		int line_count = 1;
		int left_braces = 0;
		int right_braces = 0;
		...
		List<ProgramStatementComplexity> line_inheritance_counts = new ArrayList<>();
		...
		while ((statement = br.readLine()) != null) {
			if (statement.contains(" ") && !statement.contains("\"")) {
				...
				for (String part: parts) {
					if (part.equals(ComplexitySettings.CLASS)) {
						...
						if (class_inheritance_counts.containsKey(class_name)) {
			...
			if (statement.contains("{") && isClassFound) {
				left_braces++;
			}
			
			if (statement.contains("}") && !statement.contains("\"") && isClassFound) {
				right_braces++;
			}
			...
			line_inheritance_counts.add(psc);
			
			line_count++;
		}
		
		return line_inheritance_counts;
	}
	
	public static List<ProgramStatementComplexity> getRecursionCountforStatement(ComplexityMeasurements cm, List<String> recursion_methods) throws IOException {
		BufferedReader br = cm.getBr();
		String statement = null;
		int line_count = 1;
		int left_braces = 0;
		...
		boolean isFirstLine = false;
		boolean isPotentialMethod = false;
		List<ProgramStatementComplexity> line_recursion_counts = new ArrayList<>();
		...
		while ((statement = br.readLine()) != null) {
			...
			line_recursion_counts.add(psc);
			
			line_count++;
		}
		
		return line_recursion_counts;
	}
}
